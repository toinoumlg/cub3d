---
icon: game-board-simple
---

# Texture

### Floor & Ceiling

Both colors for floor and ceiling are created here and store in `t_cub3d` as `int`\
First find in `**config`  the string starting with either `"F "`  or `"C "`  \
If a corresponding string is found we split this string with `','`  \
Then create a color with each string as a value \
While also checking if each value range `[0,255]`  and only contains digits

```c
int	check_rgb(char *rgb, t_cub3d *data)
{
	int	i;
	int	color;

	i = 0;
	if (!rgb)
		exit_error("Missing value(s) for rbg color", data);
	while (rgb[i])
		if (!ft_isdigit(rgb[i++]))
		{
			free(rgb);
			exit_error("Rbg value contain a character", data);
		}
	color = ft_atoi(rgb);
	free(rgb);
	if (color < 0 || color > 255)
		exit_error("Rgb value is outside of it's range", data);
	return (color);
}

int	set_color_from_config(char *to_find, char **config, t_cub3d *data)
{
	char	*line;
	char	**rgb;
	int		color;

	line = find_in_config(to_find, config);
	if (!line)
		exit_error("Missing rgb values in config file", data);
	line += ft_strlen(to_find);
	rgb = ft_split(line, ',');
	if (!rgb)
		exit_error("Failed to split rbg values", data);
	// not leak free
	color = create_trgb(0, check_rgb(rgb[0], data), check_rgb(rgb[1], data),
			check_rgb(rgb[2], data));
	free(rgb);
	return (color);
}
```

### Wall textures

The 4 images of walls are created here and stored in `t_cub3d` as `t_img`\
First find in `**config` the string starting with either `"NO "` , `"SO "` , `"WE "`  or `"EA "` \
Store the pointer returned from `mlx_xpm_file_to_image()`  into `img.ptr` \
Store the pointer returned from `mlx_get_data_addr()`  into `img.addr`

```c
t_img	init_texture_from_config(char *to_find, t_cub3d *data)
{
	char	*line;
	t_img	img;

	ft_memset(&img, 0, sizeof(t_img));
	line = find_in_config(to_find, data->config);
	if (!line)
		exit_error("Missing texture from config file", data);
	line += ft_strlen(to_find);
	img.ptr = mlx_xpm_file_to_image(data->mlx, line, &img.w, &img.h);
	if (!img.ptr)
		exit_error("Loading texture from file", data);
	img.addr = mlx_get_data_addr(img.ptr, &img.bpp, &img.lenght, &img.end);
	if (!img.addr)
		exit_error("Getting address from image", data);
	return (img);
}
```
