...
/*   Updated: 2026/01/13 17:42:42 by amalangu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_dir(int direction, t_double2 *dir, t_double2 *plane)
{
	...
	pi = acos(-1.0);
	if (direction == 'S')
		return (rotate(dir, plane, 3 * pi / 2.0));
	else if (direction == 'N')
		return (rotate(dir, plane, pi / 2.0));
	else if (direction == 'E')
		return (rotate(dir, plane, pi));
	else
		return ;
}
		...
		t_cub3d *data)
{
	...
	data->player.pos = set_double2(coords.x, coords.y);
